...
export default function ForumTopic() {
  ...
  const handleCreatePost = async (e) => {
    ...
    // Limpa imediatamente o campo para evitar reenvio duplicado enquanto a mutation finaliza.
    setNewPost("");
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen bg-gradient-to-br from-emerald-50 via-white to-green-50">
      <div className="max-w-4xl mx-auto">
        <Link to={createPageUrl("Forum")}>
        ...
        <Card className="shadow-lg border-emerald-100 bg-white mb-6">
          <CardContent className="p-6 md:p-8">
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm text-gray-500 pt-4 border-t border-gray-100">
              <span>Por <span className="font-medium text-gray-700">{topic.author_name || 'Comunidade'}</span></span>
              <span className="flex items-center gap-1">
                <MessageSquare className="w-4 h-4" />
                {posts.length || topic.total_posts || 0} respostas
              </span>
              <span className="flex items-center gap-1">
        ...
        <div className="space-y-4">
          ...
          {posts.map((post) => {
            const hasLiked = Array.isArray(post.liked_by) && (post.liked_by || []).includes(profile?.email || user?.email);
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <p className="font-semibold text-gray-900">{post.author_name || 'Ecoante'}</p>
                      ...
                      >
                        ...
                        {post.likes || 0} {(post.likes || 0) === 1 ? 'curtida' : 'curtidas'}
                      </Button>
                    </div>
                  </div>
                </CardContent>
