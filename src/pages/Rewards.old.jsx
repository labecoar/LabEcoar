...
import {
  Gift, Star, ShoppingCart,
  Apple, GraduationCap, Music, Heart, Smartphone, Package } from
"lucide-react";
import {
  Dialog,
  DialogContent,
...
const CATEGORY_INFO = {
  alimentacao: { name: "Alimentação", icon: Apple, color: "bg-green-100 text-green-700 border-green-200" },
  educacao: { name: "Educação", icon: GraduationCap, color: "bg-blue-100 text-blue-700 border-blue-200" },
...
export default function Rewards() {
  ...
  const { data: myClaims } = useQuery({
    ...
    queryFn: () => base44.entities.RewardClaim.filter({ user_email: user.email }, '-claimed_at'),
    initialData: [],
    enabled: !!user?.email
  });
  ...
  const claimRewardMutation = useMutation({
    mutationFn: async (reward) => {
      ...
      await base44.entities.Reward.update(reward.id, {
        quantity_claimed: (reward.quantity_claimed || 0) + 1
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['rewards'] });
      queryClient.invalidateQueries({ queryKey: ['my-claims'] });
      alert('Recompensa resgatada com sucesso! ✅\nAguarde o contato da equipe com instruções.');
  ...
  const hasEnoughPoints = (reward) => {
    return (user?.total_points || 0) >= reward.points_required;
  };

  const isAvailable = (reward) => {
    if (!reward.quantity_available) return true;
    return reward.quantity_claimed < reward.quantity_available;
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Loja de Recompensas

              </h1>
              <p className="text-gray-600 mt-2">Troque seus pontos por benefícios incríveis</p>
            </div>
        ...
        <Card className="mb-8 shadow-lg border-emerald-100 bg-gradient-to-r from-emerald-50 to-teal-50">
            ...
            <CardContent>
              ...
              <div key={claim.id} className="flex items-center justify-between p-4 bg-white rounded-lg border border-emerald-200">
                    ...
                    <Badge className={
                claim.status === 'entregue' ? 'bg-green-100 text-green-700' :
                claim.status === 'processando' ? 'bg-blue-100 text-blue-700' :
                'bg-yellow-100 text-yellow-700'
                }>
                      {claim.status === 'entregue' ? 'Entregue' :
                  claim.status === 'processando' ? 'Processando' :
                  'Pendente'}
                    </Badge>
                  </div>
              )}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-emerald-100">
                    ...
                    <Badge variant="outline" className="bg-gray-50 text-gray-500">
                        {!hasEnoughPoints(reward) ? 'Pontos insuficientes' : 'Indisponível'}
                      </Badge>
                    }
                  </div>
                  ...
                  <div className="text-xs text-gray-500">
                      {reward.quantity_available - reward.quantity_claimed} de {reward.quantity_available} disponíveis
                    </div>
                  }
                </CardContent>
              </Card>);
        ...
        <Dialog open={!!selectedReward} onOpenChange={() => setSelectedReward(null)}>
            <DialogContent className="sm:max-w-2xl">
              ...
              <div>
                    ...
                    <p className="text-gray-600">{selectedReward.validity_months} meses após o resgate</p>
                  </div>
              }
              ...
              <div>
                ...
                <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="text-sm text-gray-600">Você tem</p>
                    <div className="flex items-center gap-2">
                      <Star className="w-6 h-6 text-yellow-500 fill-yellow-500" />
                      <span className="text-3xl font-bold text-gray-900">{user?.total_points || 0}</span>
                    </div>
